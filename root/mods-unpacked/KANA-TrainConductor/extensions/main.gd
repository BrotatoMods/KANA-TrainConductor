...
func KANA_get_oposite_collision_tile_position(tile_position: Vector2, direction: Vector2) -> Vector2:
	var current_zone = ZoneService.get_zone_data(RunData.current_zone).duplicate()
	var oposite_tile_position: Vector2
	var spawn_offset = 2

	if direction == Vector2(1, 0):
		oposite_tile_position = Vector2(current_zone.width - spawn_offset, tile_position.y)
	elif direction == Vector2(-1, 0):
		oposite_tile_position = Vector2(spawn_offset, tile_position.y)
	elif direction == Vector2(0, 1):
		oposite_tile_position = Vector2(tile_position.x, current_zone.height - spawn_offset)
	elif direction == Vector2(0, -1):
		oposite_tile_position = Vector2(tile_position.x, spawn_offset)

	var local_position: Vector2 = _tile_map_limits.map_to_world(oposite_tile_position)
...
func _KANA_on_player_border_collided(collision: KinematicCollision2D) -> void:
	...
	var oposite_position := KANA_get_oposite_collision_tile_position(tile_position_collider, collision.normal)

	_player.global_position = oposite_position
